...
def image_saveto_lmdb(images_path,lmdb_path):

    total_img = os.listdir(images_path)
    num = len(total_img)
    list = range(num)

    env = lmdb.open(lmdb_path, map_size=1073741824) #size=1GB
    cache = {}  # 存储键值对
...
lmdb_path= 'lmdb_image_dir'
image_saveto_lmdb(images_path,lmdb_path)
...
### 从.npy文件读取数据
def readdatafromnpy(npy_path):
    X_train = np.load(npy_path)
    #print(X_train.shape)
    return X_train
npy_path='./lmdb_image_dir/npy/imageArr.npy'
readdatafromnpy(npy_path)

# ### 存xml标注文件到lmdb
def xml_saveto_lmdb(xml_path,lmdb_path):
...
lmdb_path= 'lmdb_xml_dir'
xml_saveto_lmdb(xml_path,lmdb_path)
